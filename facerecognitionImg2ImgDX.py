...
# Function to convert a picture in RGB to BGR (which OpenCV uses)
...
def draw_on_image(image, face_locations, face_landmarks_list):
    for face_location, face_landmarks in zip(face_locations, face_landmarks_list):
        ...
        top, right, bottom, left = int(top), int(right), int(bottom), int(left)  # Ensure the coordinates are integer
        ...
        for facial_feature in face_landmarks.keys():
            ...
            for point in points:
                cv2.circle(image, tuple(int(x) for x in point), 2, (0, 0, 255), -1)
