...
def main(reference_image_path, directory_path):
    ...
    for image_file in image_files:
        ...
        if image_encodings:
            ...
            # Convert and save the image with tags
            ...
            # Display each tagged image briefly
            cv2.imshow(f"Tagged {os.path.basename(image_file)}", tagged_image)
            cv2.waitKey(1000)  # Display each for 1000 milliseconds
            cv2.destroyWindow(f"Tagged {os.path.basename(image_file)}")
        else:
