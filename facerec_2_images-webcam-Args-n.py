...
if len(sys.argv) < 2:
    print("Usage: python script.py <image1> [<image2> ... <imageN>]")
    sys.exit(1)

# Load images from command-line arguments
image_paths = sys.argv[1:]  # List of all image file paths provided as arguments
known_face_encodings = []
known_face_names = []

for image_path in image_paths:
    image = face_recognition.load_image_file(image_path)
    # Assume each image has at least one face and take the first encoding
